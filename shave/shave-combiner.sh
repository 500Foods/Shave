...
combine_content_cst() {
    ...
    local combined_data_name="$3"   # Name of combined associative array to return (for compatibility)
    ...
    for ((i=0; i<cst_data_size; i++)); do
        ...
        if [[ "$cst_entry" =~ \[([0-9]+),\ ([0-9]+)\]\ -\ \[([0-9]+),\ ([0-9]+)\] ]]; then
            ...
            if [[ "$start_line" != "$end_line" ]]; then
                ...
                if [[ -n "$source_line" && "$start_col" -le "${#source_line}" ]]; then
    ...
    if [[ "$DEBUG_MODE" == "true" ]]; then
        local temp_combined_file=$(mktemp /tmp/shave-combined.XXXXXX.dat)
        {
        ...
        COMBINED_DEBUG_FILE="$temp_combined_file"
