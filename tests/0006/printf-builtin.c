...
// Generation Timestamp: 2026-08-18 10:56:00 PDT
...
// Hash Table Start
//
// s_0776501f: /mnt/extra/Projects/Shave/tests/0006/printf-builtin.sh
//
// Hash Table End
...
int main(int argc, char *argv[]) {
    ...
    // Generated content from /mnt/extra/Projects/Shave/tests/0006/printf-builtin.sh
    ...
    {
        const char *shave_old_TZ = getenv("TZ");
        (void)setenv("TZ", "UTC", 1);
        tzset();
        status |= shave_printf_builtin(3, (char *[]){"printf", "%(%Y-%m-%d %H:%M:%S)T\\n", "0"});
        if (shave_old_TZ != NULL) {
            (void)setenv("TZ", shave_old_TZ, 1);
        } else {
            (void)unsetenv("TZ");
        }
        tzset();
